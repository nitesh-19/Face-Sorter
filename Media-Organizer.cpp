...
get_target_face_descriptor(string path, frontal_face_detector &face_detector, shape_predictor &shape_predictor,
                           anet_type &net) {
    matrix<rgb_pixel> og_image;
    load_image(og_image, path);
    std::vector<matrix<rgb_pixel>> face;
    std::vector<matrix<rgb_pixel>> faces = face_detector(og_image);
    if (faces.size() == 1) {
        face = face_detector[0];
        cout << "Target face found, processing... " << endl;
        auto shape = shape_predictor(og_image, face);
        matrix<rgb_pixel> face_chip;
        extract_image_chip(og_image, get_face_chip_details(shape, 150, 0.25), face_chip);
        //Show image here
//        image_window(face, "image").wait_until_closed();

        std::vector<matrix<float, 0, 1>> face_descriptor = net(face);
        return face_descriptor;


    } else {
        cout << "No face or multiple faces found in the source image. Please provide image with only the target face."
             << endl;
//        return 123;
    }
}
...
void
recognize_faces(string path, frontal_face_detector &face_detector, shape_predictor &shape_predictor, anet_type &net) {
    // get path and save it to an image object
    matrix<rgb_pixel> og_image;
    load_image(og_image, path);
...
int main() {
    ...
    get_target_face_descriptor("tanay.jpg", face_detector, shape_predictor, net);
